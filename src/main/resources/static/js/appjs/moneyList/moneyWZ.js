...
function reLoad(type) {
	...
	$('#moneyWZTable').bootstrapTable('refresh');
}


function exprotWZ(){
	
	$.tzLoading({content:'正在导出乌镇电费用户，请稍等...'});
    ...
    window.location.href = prefix + '/exprotWZ?createTime='+createTime+'&userType='+userType+'&userId='+userId;
    	LoadingClose();
}
...
function batchRemove() {
	var rows = $('#moneyWZTable').bootstrapTable('getSelections'); 
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
...
function batchMoney() {
	...
	layer.confirm("确认要缴费选中的'" + rows.length + "'条数据吗?", {
		btn : [ '确定', '取消' ]
	}, function() {
		var ids = new Array();
		$.each(rows, function(i, row) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#'+id).on('change', function(e, params) {
				...
				$('#moneyWZTable').bootstrapTable('refresh', opt);
			});
		}
	});
}
