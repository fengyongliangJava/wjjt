...
function reLoad() {
	$('#elecLogTable').bootstrapTable('refresh');
}
...
function batchRemove() {
	var rows = $('#elecLogTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
...
function exprotElecLog(){
	
	$.tzLoading({content:'正在导出电费记录，请稍等...'});
    ...
    window.location.href = prefix + '/exprotElecLog?moneyDate='+moneyDate+'&userOrg='+userOrg+'&userType='+userType+'&elecType='+elecType+'&userId='+userId;
    	LoadingClose();
}
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#'+id).on('change', function(e, params) {
				...
				$('#elecLogTable').bootstrapTable('refresh', opt);
			});
		}
	});
}
