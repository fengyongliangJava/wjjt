...
function reLoad() {
	$('#waterLogTable').bootstrapTable('refresh');
}




function exprotWaterLog(){
	
	$.tzLoading({content:'正在导出水缴费记录，请稍等...'});
    ...
    window.location.href = prefix + '/exprotWaterLog?moneyDate='+moneyDate+'&userOrg='+userOrg+'&userType='+userType+'&waterType='+waterType+'&userId='+userId;
    	LoadingClose();
}
...
function batchRemove() {
	var rows = $('#waterLogTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#'+id).on('change', function(e, params) {
				...
				$('#waterLogTable').bootstrapTable('refresh', opt);
			});
		}
	});
}
